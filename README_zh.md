...
> JoltVM 正在积极开发中。Phase 1（Agent 骨架 + Attach API）、Phase 2（Netty Web Server + REST API）、Phase 3（热替换 + 回滚）、Phase 4（方法追踪 + 火焰图）和 Phase 5（Spring Boot 感知）已完成。完整计划参见[路线图](#-路线图)。
...
| `joltvm-server` | 嵌入式 Netty HTTP 服务器，提供 REST API（类列表、详情、反编译、热替换、追踪、Spring 感知） | ✅ Phase 2–5 |
