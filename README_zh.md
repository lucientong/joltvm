...
> JoltVM 正在积极开发中。Phase 1（Agent 骨架 + Attach API）、Phase 2（Netty Web Server + REST API）、Phase 3（热替换 + 回滚）、Phase 4（方法追踪 + 火焰图）、Phase 5（Spring Boot 感知）、Phase 6（Web UI）、Phase 7（安全审计）、Phase 8（安全加固）、Phase 9（线程诊断）、Phase 10（JVM 仪表盘增强）和 Phase 11（ClassLoader 分析 + Logger 动态调级）已完成。完整计划参见[路线图](#-路线图)。
