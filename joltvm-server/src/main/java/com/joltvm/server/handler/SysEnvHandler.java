...
/**
 * Handler for {@code GET /api/jvm/sysenv} — returns environment variables (sensitive values redacted).
 */
public class SysEnvHandler implements RouteHandler {

    private final JvmInfoService jvmInfoService;

    public SysEnvHandler(JvmInfoService jvmInfoService) {
        this.jvmInfoService = jvmInfoService;
    }

    @Override
    public FullHttpResponse handle(FullHttpRequest request, Map<String, String> pathParams) {
        return HttpResponseHelper.json(jvmInfoService.getEnvironmentVariables());
    }
}
