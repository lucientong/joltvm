...
/**
 * Handler for {@code GET /api/jvm/gc} — returns GC statistics.
 */
public class GcStatsHandler implements RouteHandler {

    private final JvmInfoService jvmInfoService;

    public GcStatsHandler(JvmInfoService jvmInfoService) {
        this.jvmInfoService = jvmInfoService;
    }

    @Override
    public FullHttpResponse handle(FullHttpRequest request, Map<String, String> pathParams) {
        return HttpResponseHelper.json(jvmInfoService.getGcStats());
    }
}
