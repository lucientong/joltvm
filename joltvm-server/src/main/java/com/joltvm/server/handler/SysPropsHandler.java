...
/**
 * Handler for {@code GET /api/jvm/sysprops} — returns system properties (sensitive values redacted).
 */
public class SysPropsHandler implements RouteHandler {

    private final JvmInfoService jvmInfoService;

    public SysPropsHandler(JvmInfoService jvmInfoService) {
        this.jvmInfoService = jvmInfoService;
    }

    @Override
    public FullHttpResponse handle(FullHttpRequest request, Map<String, String> pathParams) {
        return HttpResponseHelper.json(jvmInfoService.getSystemProperties());
    }
}
